...
def get_rag_chain():
    ...
    system_prompt = (
        ...
        "아래에 제공된 문서를 활용해서 답변해주시고, 문서내에서 답변을 알 수 없다면 모른다고 답변해주세요. 가독성이 좋게 볼드체나 개조식 등을 사용해서 답변해주세요."
        "\n\n"
        "{context}"
    )
...
def get_ai_response(user_message):
    ...
    # # Retriever에서 문서 검색
    # retriever = get_retriever()
    # retriever_results = retriever.invoke(user_message, k=3)  # k는 반환할 문서 수

    # # 검색된 문서 출력
    # print("Retriever 검색 결과:")
    # for doc in retriever_results:
    #     print(doc.page_content)  # Document 객체의 내용을 출력
    ###############################################################################
