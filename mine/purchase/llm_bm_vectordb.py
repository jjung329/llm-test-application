...
def compare_results(bm25_results, retriever_results):
    print("BM25 검색 결과:")
    ...
    # 유사성 비교 예시
    # 이 부분에서 특정 기준을 정해 비교할 수 있습니다.
    # 예: 같은 키워드가 포함된 문서 확인
    bm25_set = set(bm25_results)
    retriever_set = {doc.page_content for doc in retriever_results}

    common_docs = bm25_set.intersection(retriever_set)
    print("\n공통 문서:")
    for doc in common_docs:
        print(doc)


def calculate_similarity(bm25_results, retriever_results):
    # 모든 문서를 하나의 리스트로 합침
    all_documents = bm25_results + [doc.page_content for doc in retriever_results]
    
    # TF-IDF 벡터화
    vectorizer = TfidfVectorizer().fit_transform(all_documents)
    vectors = vectorizer.toarray()

    # 유사도 계산
    cos_sim = cosine_similarity(vectors)
    
    # BM25 결과와 Retriever 결과의 유사도 출력
    for i in range(len(bm25_results)):
        for j in range(len(retriever_results)):
            print(f"BM25 문서 {i+1}과 Retriever 문서 {j+1}의 유사도: {cos_sim[i][len(bm25_results) + j]}")
...
def get_ai_response(user_message):
   ...
   # retriever를 사용하여 검색한 문서 가져오기
    retriever = get_retriever()
    retriever_results = retriever.get_relevant_documents(user_message, limit=5)  # retriever로 검색한 문서
    retriever_context = "\n\n".join([doc.page_content for doc in retriever_results])  # retriever 결과를 문맥으로 변환 

    # 두 문맥을 결합
    # combined_context = bm25_context + "\n\n" + retriever_context
    # 가중치 조정된 문서 결합
    # combined_context = weighted_combination(bm25_results, retriever_results)
    combined_context = weighted_combination(bm25_results, retriever_results)
    ...
    # print("\nRetriever 검색 결과:=============================================================================")
    # for doc in retriever_results:
    #     print(doc.page_content)  # Document 객체에서 내용 출력

   # 검색 결과 비교
    # compare_results(bm25_results, retriever_results)
    # calculate_similarity(bm25_results, retriever_results)
